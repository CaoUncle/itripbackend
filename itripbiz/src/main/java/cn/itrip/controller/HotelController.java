...
import cn.itrip.service.ItripAreaDicService;
...
public class HotelController {
    ...
    public Dto<ItripAreaDicVO> queryHotCity(@PathVariable Integer type) {
        List<ItripAreaDic> itripAreaDics = null;  //ItripAreaDic对应数据库中的市区表
        List<ItripAreaDicVO> itripAreaDicVOs = null;   //ItripAreaDicVO返回前端-(ItripAreaDic)区域VO
        try {
            if (EmptyUtils.isNotEmpty(type)) {  //如果type不为空
                Map param = new HashMap();
                param.put("isHot", 1);
                param.put("isChina", type);
                itripAreaDics = itripAreaDicService.getItripAreaDicListByMap(param);  //按照isChina:国内 isHot:国外查询数据
                if (EmptyUtils.isNotEmpty(itripAreaDics)) {  //如果查到的数据不为空
                    itripAreaDicVOs = new ArrayList();
                    for (ItripAreaDic dic : itripAreaDics) {  //遍历查询到的数据
